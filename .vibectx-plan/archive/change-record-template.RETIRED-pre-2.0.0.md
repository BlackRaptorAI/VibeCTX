> **RETIRED 2026-09-08 — DO NOT USE THIS TEMPLATE. It will fail validation.**
>
> This is the **pre-2.0.0** verdict shape. Pack 2.0.0 changed it: `confidence` is an integer 0–10
> (not `"high"`/`"medium"`/`"low"`), `standards` is **required**, `evidence` is a **string** not an
> array, and the `N/A` verdict was **removed** — a gate that does not apply emits no block and the
> Change Record row carries the N/A.
>
> **Take the current template from the pack's `change-record` skill.** See **D-53**.
>
> Kept only to explain why the 13 pre-existing Change Records do not validate.

# Change Record — CR-YYYYMMDD-<short-slug>

<!--
USAGE
  1. Copy this file to .vibectx-plan/change-records/CR-YYYYMMDD-<slug>.md IN THE SAME PR
