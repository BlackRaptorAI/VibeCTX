...
>
> The workflows and required checks this checklist configured were removed from the repository and
> its history by `a0852f2`. Branch handoff is now by `git bundle` into `../vibectx-handoff-bundles/`,
> landed by Tom from his own terminal — **no agent session pushes, as a control rather than a
> capability limit.** See **D-54**.

# Branch Protection Checklist — Solo Author Mode

Target: `main` on `BlackRaptorAI/BlackRaptorAI/VibeCTX`.
Context: written for a solo coder (merge to main runs CI (lint · test · build); a `v*` tag marks a release — VibeCTX is distributed as source, so the tag IS the artifact and consumers pin by checking that tag out of their clone);
...
- [ ] Create team **(none yet — see .vibectx-plan/branch-protection-checklist.md)**, add **your second approver only**, grant
...
- [ ] **Required approvals: 0.** Not a typo. With 1+, every PR needs an
      ...
      the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR-gating comes from the next toggle, not this number.
- [ ] **Require review from Code Owners: ON.** With the sparse CODEOWNERS,
      only Tier-3 paths request the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR; everything else merges without review.
      ...
      a Tier-3 path should be blocked until the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR approves.
...
- [ ] **Dismiss stale pull request approvals when new commits are pushed: ON.**
      Otherwise you could get the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR's approval, then push different code and
...
- [ ] **Require conversation resolution before merging: ON.** Cheap; makes
      the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR's questions un-ignorable.
...
- Emergency path (prod down, the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR unreachable): temporarily disable the rule
...
- [ ] Test PR touching a Tier-3 path → blocked until the founder (Tom) — no second approver exists yet, so a Tier-3 change records its own risk acceptance in the CR approves; approval
