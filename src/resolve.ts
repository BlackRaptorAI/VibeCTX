 ...
 *      then PyPI `pypi.org/pypi/<name>/json`. npm first; a name both know (`stripe`,
 *      `httpx`) resolves to the npm package unless `ecosystem: "pypi"` is given.
 ...
 *   4. GitHub README: `raw.githubusercontent.com/<o>/<r>/main/README.md`, then `master`.
 ...
 * MAX_README_CANDIDATES (2) = 12. Candidates are probed by getLibraryDoc, which stops
...
export const MAX_README_CANDIDATES = 2;
 ...
 *  origin, then the homepage's; then the GitHub README on `main`, then `master`. */
export function synthesizeCandidates(meta: PackageMetadata): string[] {
  ...
  const readme = meta.repository
    ? ["main", "master"].map((b) => `https://raw.githubusercontent.com/${meta.repository!.owner}/${meta.repository!.repo}/${b}/README.md`)
    : [];
...
export async function resolvePackage(
  ...
  const order: Ecosystem[] = ["npm", "pypi"];
  let fetched = 0;
  for (const eco of order) {
    ...
    if (!doc) {
      attempts.push(
        `${label} metadata found (${describeMeta(meta)}); none of ${candidates.length} candidate URLs served a document: ${candidates.join(", ")}`,
      );
      return fail(); // candidates were probed: the fetch budget for this name is spent
    }
