 ...
 *
 * Runs at AUTOWARM_CONCURRENCY. index.ts starts it with `void startAutowarm(...)` AFTER
 * the stdio transport is connected, so it never delays the handshake and never blocks a
 * tool call (fetches are async I/O; a concurrent get_docs for the same entry simply
 * fetches too — both write the same content). Every error is swallowed into one summary
 * line on stderr; the server cannot crash because of it. Opt out with
...
/** Off when `VIBECTX_NO_AUTOWARM` is set to anything but "", "0" or "false", or when the
 *  server was started with `--offline`. Subcommands never reach this: index.ts only calls it
 *  on the server path. */
export function shouldAutowarm(env: NodeJS.ProcessEnv, argv: readonly string[]): boolean {
...
export async function startAutowarm(
  ...
  const summary: AutowarmSummary = { attempted: 0, cached: 0, failed: [] };
  const errors: string[] = [];
  try {
  ...
  if (summary.attempted > 0) {
    ...
    try {
      warn(`vibectx: autowarm cached ${summary.cached}/${summary.attempted} configured libraries${failed}${detail}\n`);
    } catch {
